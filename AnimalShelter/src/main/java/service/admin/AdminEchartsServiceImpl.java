...
@Service("adminEchartsService")
@Transactional
public class AdminEchartsServiceImpl implements AdminEchartsService {

    @Autowired
    private AdminEchartsDao adminEchartsDao;

    @Override
    public Map<String,Object> kindCount() {
        List<KindEcharts> list = adminEchartsDao.kindCount();

        String[] name = new String[list.size()];
        String[] data = new String[list.size()];
        Map<String,Object> map = new HashMap<>();
        for (int i=0;i<list.size();i++){
            name[i] = list.get(i).getKindname();
            data[i] = Integer.toString(list.get(i).getKindcount());
            map.put("name", name);
            map.put("data", data);
        }
